...
struct GatheringDetailView: View {
    ...
    private var currentSummary: GatheringSummary {
        (manager.openSummaries + manager.mySummaries).first { $0.gathering.id == summary.gathering.id } ?? summary
    }
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingApplySheet) {
            GatheringApplySheet(gathering: currentSummary.gathering) { comment in
                Task {
                    let succeeded = await manager.apply(to: currentSummary.gathering, comment: comment)
                    toastMessage = succeeded ? "応募しました" : "応募できませんでした"
                }
            }
        }
...
private struct GatheringApplySheet: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section {
                    TextField("一言コメント(任意)", text: $comment, axis: .vertical)
                        .lineLimit(3...6)
                } header: {
