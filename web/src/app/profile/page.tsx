  ...
  return (
    ...
    <main className="mx-auto w-full max-w-lg flex-1 px-5 py-7 sm:px-8">
      <PageHeader
        ...
        subtitle={university ? `🎓 ${university.name}` : undefined}
        action={
          <button onClick={handleSignOut} className="text-sm font-semibold text-gray-400 hover:text-gray-600">
            {t("profile.logout")}
          </button>
        }
      />
      ...
      {userId && <MyPageExtras profile={profile} userId={userId} onProfileChange={setProfile} />}
    </main>
    <NavBar />
    </div>
  );
